...
- **Dashboard Device Info Resolution Fix:** Fixed false "No Device Connected" warning when clicking calibration buttons in `DashboardView` (`src/gui_v2/views/dashboard_view.py`). Implemented `get_active_device_info()` to resolve active controller metadata dynamically from `self._current_device_info`, `self.decision_engine.active_device_info`, or active profile settings in `config.ini`, allowing selective and full calibration wizards to launch immediately when connected.
