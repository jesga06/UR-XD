...
- **Native GUI Calibration Wizard Overhaul:** Completely rewritten `NativeCalibrationWizardDialog` (`src/gui_v2/dialogs/calibration_wizard_dialog.py`). Features 15-second high-frequency XInput C-API auto-detection as Step 0 with countdown timer, constant polling, and an explicit **"Skip / DInput Mode"** button. Implemented thread-safe HID report signal delivery (`hid_report_received`), real-time raw HID byte/bitmask XOR delta detection (`curr ^ base`) for digital button inputs, active masking of previously mapped button bitmasks (`unmapped_diff = raw_diff & ~known_mapped_bitmasks`) to allow prompt progression while holding previous buttons, 800ms-1.0s prompt reading cooldowns, analog noise filtering (`len(byte_history) > 3`), 3-click confirmation counter (`click_counts`) for L3/R3 with 0.4s debounce, 4-bit nibble D-Pad Hat switch detection (`clean_hat_bytes` -> `type: hat`), 2.0s continuous trigger sampling window with `max_uniques` state analysis and digital trigger fallback, axis inversion (`invert`), signed math (`signed`), `centeredness`, and `range_confidence` calculations, single-step "Undo Last Input" buttons, physical extra buttons entry (`M1`, `M2`, `L4`, `R4`), layout selection (`Xbox`, `PlayStation`, `Nintendo`), skip button options, and automatic profile generation in `profiles/{VID}_{PID}.json`.
