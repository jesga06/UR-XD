...
- **Legacy Macro Builder Restoration:** Rebuilt the Macro Builder in `AdvancedView` matching the legacy screenshot layout 1:1 with `Name:` field, `Outputs:` sequence field, `[Rec]` record button, `[X]` red delete button, `+ Add Macro`, `Save Settings`, and 100% two-way persistence to `macros.json`.
