...
- **Modular Documentation Directory (`docs/`):** Restructured project documentation by creating a dedicated `docs/` directory housing topic-focused guides: Getting Started, Visual GUI User Guide, Calibration, HID Maps, Macros Studio, Hardware Chords, Troubleshooting, Architecture & Pipeline, Developer Guide, and FAQ.
...
## [2.4.0] - 2026-07-25
### 🚀 Added / Implemented (PySide6 Revamp)
- **Phase 4, Slice 2 (Remapping View & Key Recorder Dialog):** Implemented `RemappingView` (`src/gui_v2/views/remapping_view.py`) and `KeyRecorderDialog` (`src/gui_v2/dialogs/key_recorder_dialog.py`). Features full shift layer management (add/rename/delete/select) with compact 2-row layout, 4 categorized button mapping grids (Face, Shoulders/Sticks, D-Pad, System/Extras), dual independent XInput block checkboxes (`Blk` for Standard Map & `S.Blk` for Shift Map) with auto-enable & session double-input warning notice, restored Home+Hold shift trigger warning, interactive pynput & Qt multi-key/mouse/scroll recorder with key normalization, Windows hardware Media Key capture (`media_volume_up`, `media_volume_down`, `media_volume_mute`, `media_play_pause`, `media_next`, `media_previous`, `media_stop`), robust Shift modifier combination handling (`Shift+Tab`, `Shift+1`, `Shift+A`, `Shift+Symbol` symbol un-shifting), rising-edge gamepad telemetry capture, thread-safe Qt Signal dispatching, and 300ms debounced disk IO.
- **Phase 4, Slice 3 (Analog Tuning View & Circularity Calibration Modal):** Implemented `TuningView` (`src/gui_v2/views/tuning_view.py`) and `CircularityCalibrationModal` (`src/gui_v2/dialogs/circularity_modal.py`) with 100% v2.3-beta feature parity. Features dual side-by-side canvases per Stick Card (1D 180x180 magnitude curve graph + 2D 180x180 crosshair radar with dual Cyan Raw vs Green Processed live dots), dual side-by-side canvases per Trigger Card (1D 180x180 curve graph + 60x180 vertical actuation meter with dual Raw vs Processed fill bars), interactive mouse drag-and-drop dotted curve control point editing with 2-8 dot selector, horizontal drag sliders (`QSlider`) with decimal text readouts (`0.00`), inline Circularity Mode dropdown selector (`disabled`, `before`, `after`), 60Hz 4-state circularity calibration sweep machine (`REST` -> `WAIT_SWEEP` -> `SWEEP` -> `DONE`), top `🎨 Color Guide` signal legend modal, LaTeX formula & copyable JSON snippet exporter, `↺ Reset Defaults` button per card, full stick-parity curve presets & sliders for Triggers, borderless enlarged dot aesthetics (+20% radius), dynamic red deadzone circle overlays on all Dashboard & Tuning radar visualizers, dotted vector trailing lines matching output dot color, Dashboard stick radars routing tuned modified output coordinates, Neon Green (`#00f5a0`) live curve cursor dots matching output signal, and 300ms debounced disk IO.
...
- **Shift Layer Modifier Matching & Selector Formatting Fix:** Fixed shift modifier key matching so single trigger shift layers (e.g. `[HOME]`) and trigger+modifier chord layers (e.g. `[HOME+RB]`) resolve correctly without defaulting to the last created shift layer. Normalized `'none'` and `'null'` modifier strings across `mapper.py`, `config_manager.py`, and `remapping_view.py`, and updated the GUI layer selector dropdown to display live chord indicators (e.g. `Shift Layer 1 [HOME+RB]`).
...
- **Phase 4, Slice 2 (Remapping View & Key Recorder Dialog):** Implemented `RemappingView` (`src/gui_v2/views/remapping_view.py`) and `KeyRecorderDialog` (`src/gui_v2/dialogs/key_recorder_dialog.py`). Features full shift layer management (add/rename/delete/select) with compact 2-row layout, 4 categorized button mapping grids (Face, Shoulders/Sticks, D-Pad, System/Extras), dual independent XInput block checkboxes (`Blk` for Standard Map & `S.Blk` for Shift Map) with auto-enable & session double-input warning notice, restored Home+Hold shift trigger warning, interactive pynput & Qt multi-key/mouse/scroll recorder with key normalization, Windows hardware Media Key capture (`media_volume_up`, `media_volume_down`, `media_volume_mute`, `media_play_pause`, `media_next`, `media_previous`, `media_stop`), robust Shift modifier combination handling (`Shift+Tab`, `Shift+1`, `Shift+A`, `Shift+Symbol` symbol un-shifting), rising-edge gamepad telemetry capture, thread-safe Qt Signal dispatching, and 300ms debounced disk IO.
- **Phase 4, Slice 3 (Analog Tuning View & Circularity Calibration Modal):** Implemented `TuningView` (`src/gui_v2/views/tuning_view.py`) and `CircularityCalibrationModal` (`src/gui_v2/dialogs/circularity_modal.py`) with 100% v2.3-beta feature parity. Features dual side-by-side canvases per Stick Card (1D 180x180 magnitude curve graph + 2D 180x180 crosshair radar with dual Cyan Raw vs Green Processed live dots), dual side-by-side canvases per Trigger Card (1D 180x180 curve graph + 60x180 vertical actuation meter with dual Raw vs Processed fill bars), interactive mouse drag-and-drop dotted curve control point editing with 2-8 dot selector, horizontal drag sliders (`QSlider`) with decimal text readouts (`0.00`), inline Circularity Mode dropdown selector (`disabled`, `before`, `after`), 60Hz 4-state circularity calibration sweep machine (`REST` -> `WAIT_SWEEP` -> `SWEEP` -> `DONE`), top `🎨 Color Guide` signal legend modal, LaTeX formula & copyable JSON snippet exporter, `↺ Reset Defaults` button per card, full stick-parity curve presets & sliders for Triggers, borderless enlarged dot aesthetics (+20% radius), dynamic red deadzone circle overlays on all Dashboard & Tuning radar visualizers, dotted vector trailing lines matching output dot color, Dashboard stick radars routing tuned modified output coordinates, Neon Green (`#00f5a0`) live curve cursor dots matching output signal, and 300ms debounced disk IO.
- **Phase 8 (Advanced Dynamic Theme Engine & Derived Color Pipeline):** Refactored `ThemeManager` (`src/gui_v2/services/theme_manager.py`), `CustomizationView` (`src/gui_v2/views/customization_view.py`), and `ThemePreviewWidget` (`src/gui_v2/widgets/theme_preview_widget.py`). Enforces strict classification separating 4 Exposed Base Colors (`window_bg`, `accent_1`, `accent_2`, `text`) from mathematically calculated Derived Colors (`widget_bg`, `graph_bg`, `graph_axis`, `outline`, `button_bg`, `button_hover`, `button_pressed`, `tab_active`, `tab_inactive`). Added 3 Theme Behavior Source Selectors (`button_color_source`, `widget_bg_source`, `outline_source`), 2 interactive Brightness Sliders (`widget_brightness`, `graph_brightness` scaling from -80% to +80%), HSV mathematical brightness transform `adjust_brightness()`, real-time signal propagation `theme_changed(dict)` triggering instant repaints and QSS updates across registered widgets and QPainter canvases without restarting, JSON theme import/export supporting full base colors, toggles, and brightness slider state with legacy schema fallback, 100% enforcement of hardware input visualizers using Accent #1 and virtual output visualizers using Accent #2, and full unit test coverage.
- **Shift Layer Modifier Matching & Selector Formatting Fix:** Fixed shift modifier key matching so single trigger shift layers (e.g. `[HOME]`) and trigger+modifier chord layers (e.g. `[HOME+RB]`) resolve correctly without defaulting to the last created shift layer. Normalized `'none'` and `'null'` modifier strings across `mapper.py`, `config_manager.py`, and `remapping_view.py`, and updated the GUI layer selector dropdown to display live chord indicators (e.g. `Shift Layer 1 [HOME+RB]`).
...
## [2.5.0] - 2026-07-30
### 🚀 Added / Implemented (Phase 5 & 6 Core Performance, Dynamic Connection & Calibration)
- **Phase 6 (Dynamic Connection Engine & GUI Calibration Wizard):** Implemented explicit 6-state connection FSM (`DISCONNECTED`, `WAITING`, `CONNECTING`, `CONNECTED`, `DISCONNECTING`, `INIT_FAILED`) in `src/backend_base.py` and `src/main.py`. Refactored `DashboardView` (`src/gui_v2/views/dashboard_view.py`) with a dual-state architecture: State A (WAITING View) embedding `DevicePickerWidget` (`src/gui_v2/widgets/device_picker_widget.py`) for system USB HID device enumeration and non-latching manual selection, and State B (CONNECTED View) for standard live telemetry monitoring. Integrated `TransitionOverlayWidget` (`src/gui_v2/widgets/transition_overlay_widget.py`) providing color-interpolated canvas opacity cross-fades (250ms fade-in, 900ms hold, 250ms fade-out) with vector loading spinner and loading screen quotes (`.loading_quotes.json`). Implemented `ProfileDecisionEngine` (`src/gui_v2/services/auto_calibration_service.py`) executing automated profile resolution decision trees (XInput mode check -> local profile -> community database -> wizard launch). Implemented `NativeCalibrationWizardDialog` (`src/gui_v2/dialogs/calibration_wizard_dialog.py`) replacing legacy CLI calibration scripts with a native PySide6 multi-step wizard, backend math/parsing reuse (`src/calibration.py`), progress bar tracking, layout selection, baseline rest capture, button/axis checks, and automatic XInput C-API hardware switch verification (`verify_xinput_switch()`).
- **Decoupled 1000Hz IPC Telemetry Worker:** Implemented `UDPTelemetryWorker` (`src/gui_v2/workers/telemetry_worker.py`) using `QThread` and `QMutex` thread-safe atomic state snapshots, completely decoupling high-frequency backend UDP packet ingestion from GUI frame rendering.
...
- **2-Key Priority Shift Layer Resolution Engine:** Implemented deterministic 2-key shift layer priority evaluation in `mapper.py`. 2-key chord layers (`trigger` + `modifier`) take precedence over single-trigger layers (`trigger` only) when both keys are physically pressed, while single-trigger layers match when only the trigger key is held down without modifiers. Added base mapping fallback for unmapped buttons on active shift layers.
- **Dynamic Per-Layer XInput Suppression:** Updated `virtual_pad.py` to evaluate button blocks dynamically based on `mapper.active_layer` and active shift trigger/modifier inputs. Standard blocks (`Blk`) apply on `layer_base`, while Shift blocks (`S.Blk`) apply exclusively on active shift layers, preventing XInput block bleed-through.
- **Algorithmic Shift Layer Deduplication & Selection Fix:** Added automatic algorithmic layer ID repair and unique ID generation in `config_manager.py` to repair duplicate layer IDs and prevent collisions on layer deletion. Fixed `8bitdo_ultimate_2c_wireless_controller.json` layer IDs, and updated `RemappingView` in `remapping_view.py` to prioritize index-based layer lookup so GUI layer dropdowns correctly reflect each selected layer's remappings.
- **UnboundLocalError & Signal Disconnect Fixes:** Resolved `UnboundLocalError` in `mapper.py` by defining `base_map` prior to the layer transition held-button replay loop, and fixed `TypeError` in `circularity_modal.py` by wrapping PySide6 `clicked.disconnect()` safely.
- **Circularity Calibration Modal Performance & Dynamic Theme Refactor:** Optimized `CircularityCalibrationModal` and `CircularityPolarCanvas` (`src/gui_v2/dialogs/circularity_modal.py`) with 360-degree pre-computed trigonometric lookup tables (`_COS_TABLE`, `_SIN_TABLE`), `_set_status` throttled label updates to prevent 60Hz Qt text layout recalculations, pre-cached `QPen`/`QBrush` styling objects, and full `ThemeManager` integration (`accent_1`, `accent_2`, `background`, `window_bg`). Updated `tuning_view.py` and `remapping_view.py` to correctly resolve `telemetry_worker.telemetry_updated` signals for modal live coordinate streaming.
- **Input Blocking & Toggle Chord Layer Precedence Fix:** Fixed `virtual_pad.py` block preference resolution to evaluate all explicit `block_xinput` entries for base and shift layers regardless of mapping presence. Exposed `consumed_shift_buttons` on `Mapper` to suppress active shift trigger/modifier buttons on XInput, and prioritized 2-key chord toggle layers over single-trigger toggle layers to eliminate layer collision and accidental layer deactivation.
- **Phase 4, Slice 4 (Advanced & Utilities Views, Parity & Diagnostics):** Implemented `AdvancedView` (`src/gui_v2/views/advanced_view.py`), `UtilitiesView` (`src/gui_v2/views/utilities_view.py`), `ChordsGuideDialog` (`src/gui_v2/dialogs/chords_guide_dialog.py`), `RemappingGuideDialog` (`src/gui_v2/dialogs/remapping_guide_dialog.py`), `HomeWarningDialog` (`src/gui_v2/dialogs/home_warning_dialog.py`), and `CircularityInfoDialog` (`src/gui_v2/dialogs/circularity_info_dialog.py`). Achieved 100% operational and visual parity against legacy branch `v2.3-beta` (`src/gui.py`). Features firmware-level Hardware Chords Builder with input suppression and synthetic extra button synthesis (`M1`, `M2`, `L4`, `R4`), XInput backend mode warning banner, Macros Studio with sequence recording (`KeyRecorderDialog`), 3 execution modes (`one_shot`, `toggle`, `hold`), live 2Hz telemetry counter panel (Polling Rate Hz, Avg Processing Latency ms, Max Latency Delta ms), 10,000 packet synthetic math benchmark, diagnostic suite launchers (`tools_and_diagnostics.bat`, `input_graph.py`), asynchronous background Community Profiles Fetcher (`community_fetcher.py`), debounced disk I/O (`DebouncedConfigSaver`), and zero-hardcoded-color dynamic `ThemeManager` QSS styling.
- **Live Macro & Hardware Chord Synchronization Fixes:** Updated `MacroExecutor.load_macros()` (`src/macro_executor.py`) to clear and overwrite cached in-memory macro definitions upon reload, fixing stale macro execution when step sequence or execution mode is modified. Added `config_updated` signal on `AdvancedView` connected to `DashboardView.button_matrix.load_profile_schema()` and `RemappingView.reload_extra_buttons_grid()`, ensuring newly added or deleted hardware chords (`M1`, `M2`, `L4`, `R4`) immediately populate/depopulate on the Dashboard button matrix and Remapping grid without requiring an app restart.
- **Macro Hold Mode Release Fix:** Forwarded button release events in `mapper._release()` (`src/mapper.py`) to `MacroExecutor.on_button_release()`. Releasing a gamepad button mapped to a macro in `hold`/`loop` mode now immediately interrupts sequence execution and resets state, distinguishing hold mode cleanly from toggle mode.
- **Hardware Chords DInput Mode Lock Fix:** Updated `AdvancedView.check_backend_mode()` (`src/gui_v2/views/advanced_view.py`) to query section `[backend]` key `mode` accurately from `ControllerConfig`. When the controller is in DInput mode, the warning banner is displayed and `hw_rows_container` and `btn_add_hw` are disabled (`setEnabled(False)`), preventing invalid hardware chord creation outside XInput mode.
- **Native GUI Calibration Wizard Overhaul:** Completely rewritten `NativeCalibrationWizardDialog` (`src/gui_v2/dialogs/calibration_wizard_dialog.py`) powered by a new decoupled `CalibrationEngine` service (`src/gui_v2/services/calibration_engine.py`). Ports the exact `_calibrate_loop` state machine from `calibration.py:630-1065` verbatim, including per-step baseline re-synchronization (`self.baselines`), `known_bits` and `known_axis_bytes` candidate filtering (isolating `lx`, `ly`, `rx`, `ry`, `lt`, and `rt` byte indices dynamically), `len(byte_history) > 3` analog noise rejection, 3-click L3/R3 confirmation with 0.4s debounce, 4-bit nibble D-Pad Hat switch detection (`clean_hat_bytes`), 2.0s continuous trigger sampling window with `max_uniques` state analysis and digital trigger fallback, axis candidate sorting by amplitude (`norm_amp`), axis inversion (`invert`), signed math (`signed`), `centeredness`, and `range_confidence` calculations, single-instance engine lifecycle (`reconfigure()`) restoring full single-step Undo/Skip navigation, 15-second XInput C-API auto-detection, physical extra buttons entry (`M1`, `M2`, `L4`, `R4`), layout selection (`Xbox`, `PlayStation`, `Nintendo`), and automatic profile generation in `profiles/{VID}_{PID}.json`.
- **Calibration Release Detection & Settled Pause:** Enhanced `CalibrationEngine._advance_step()` with active button release detection and dynamic UI prompt updates. When an input is registered, prompt and status text instantly turn bright yellow/orange (`#FFAA00`) instructing the user to `"RELEASE {BUTTON}..."`, polling raw HID reports until inputs return to baseline (up to 1.5s timeout) followed by an extended 0.8s rest settling delay and non-blocking `QApplication.processEvents()` ticks, preventing accidental button skips on D-Pad and extra button steps.
- **Calibration Undo Cascade Skip Fix:** Fixed automatic step-skipping when clicking "Undo" in `CalibrationEngine`. `undo_step()` now re-synchronizes `baselines` against the current physical rest state payload and engages a 0.8-second `ignore_until_time` input cooldown, preventing residual HID report diffs from instantly triggering auto-advance cascades on undone steps.
- **Stick Auto-Calibration Cascade Fix & Interactive Radar Verification:** Increased stick detection amplitude threshold from `amp8 > 10` to `amp8 > 40` (~15.6% movement) and added a 1.0-second post-axis cooldown in `CalibrationEngine`, preventing thumbstick recoil from triggering accidental auto-advances. Added dedicated interactive stick telemetry verification views with embedded `StickRadarWidget` (180x180 canvas) right after calibrating Left and Right stick X/Y axes, complete with real-time vector decoding, `"Confirm & Continue"` progression, and a `"↩ Re-do Stick"` button that rewinds calibration state directly to the target stick's first prompt.
- **Extra Button Calibration Noise Reset & Profile Persistence:** Fixed extra button (`L4`, `R4`, `M1`, `M2`) step skipping caused by `button_byte_history` noise accumulation across steps in `CalibrationEngine`. `_emit_current_prompt()` now clears `button_byte_history` at the start of every step, allowing extra buttons sharing byte locations to be mapped cleanly. `NativeCalibrationWizardDialog._save_profile_and_finish()` now populates top-level `"extra_buttons"` in the profile JSON, updates `config.ini` `last_profile`, and triggers `reload_extra_buttons_grid()` so newly calibrated extra buttons immediately display in the Dashboard button matrix and Remapping grid.
- **Calibration Engine Diagnostic Telemetry Logging (Blueprint A):** Implemented verbose, real-time diagnostic logging in `CalibrationEngine` (`src/gui_v2/services/calibration_engine.py`). Emits timestamped console logs detailing raw HID report payloads, input diffs `[(fid, byte, curr, base)]`, suppressed report counts during `ignore_until_time` cooldowns, release verification latencies, rest state settling completions, and step index transitions, providing complete empirical log visibility into report queueing behavior during wizard calibration.
- **Rest Baseline Contamination Protection:** Fixed post-D-Pad / button step cascade by safeguarding `self.baselines` in `_advance_step()` and `undo_step()` (`src/gui_v2/services/calibration_engine.py`). Rest baselines are now updated ONLY when all report bytes have returned to clean rest state (`final_diff_count == 0`), preventing dirty pressed states (e.g., D-Pad UP byte 3 = 0) from corrupting the baseline and generating permanent false diffs (`('0_1', 3, 15, 0)`) across subsequent steps.
- **Button Active Bitmask Isolation & Step Overflow Guard:** Fixed extra button `L4` / `R4` auto-advance cascade in `CalibrationEngine` (`src/gui_v2/services/calibration_engine.py`). Replaced bitmask XOR diff math (`curr_val ^ base_val`) with active bit activation (`curr_val & ~base_val`), isolating strictly bits that turn ON during physical button presses and preventing rest baseline bits from triggering false matches on adjacent steps (`R4`). Added step index boundary check `if self.current_step_idx >= len(self.steps): return` at top of `_advance_step()`, eliminating post-completion step overruns.
- **Selective Input Calibration Feature:** Added a **`"🎯 Selective Calibration"`** action button to the Dashboard header card (`src/gui_v2/views/dashboard_view.py`) and implemented `SelectiveCalibrationDialog` (`src/gui_v2/dialogs/selective_calibration_dialog.py`). Enables users to pick specific buttons, thumbsticks, triggers, D-Pad, or extra paddles for recalibration, or type in new extra button names (e.g., `C`, `Z`, `P1`, `P2`) on the fly. Launches `NativeCalibrationWizardDialog` in selective mode, bypassing setup screens, performing a quick rest baseline capture, running calibration strictly for selected keys, and merging updated mappings directly into the controller's existing profile JSON.
- **Dashboard Device Info Resolution & Live HID Discovery:** Replaced hardcoded device fallbacks in `DashboardView` (`src/gui_v2/views/dashboard_view.py`) and `ProfileDecisionEngine` (`src/gui_v2/services/auto_calibration_service.py`) with dynamic live USB HID device enumeration via `HIDReader.get_all_devices()`. `get_active_device_info()` now retrieves real physical controller metadata (`vendor_id`, `product_id`, `product_string`, `path`) directly from the OS, eliminating all hardcoded VID/PID fallbacks and guaranteeing calibration wizards launch cleanly with real device attributes.
- **Selective Recalibration & Hat Baseline Normalization:** Fixed selective button recalibration in `CalibrationEngine` (`src/gui_v2/services/calibration_engine.py`). `known_bits` filtering now excludes the target input being recalibrated (`if in_name != name`), enabling existing profile inputs (e.g. `start`) to be re-mapped cleanly. Added a re-entrancy lock `self.step_in_progress` to prevent queued PySide Qt event ticks from triggering concurrent `_advance_step()` loops during settling delays. Normalized Hat switch rest nibbles (`base_nib = 15`), ensuring D-Pad UP presses register reliably even if rest baselines were captured while inputs were held.
- **Phase 7 (Standardized Severity Logging System, Log Export Sanitation & Event-Driven Personality Quote Engine):** Refactored logger architecture in `src/logger_setup.py` adding custom log levels (`SUCCESS` at Level 22, `QUOTE` at Level 25), standardized bracketed severity tags (`[INFO]`, `[SUCCESS]`, `[WARN]`, `[ERROR]`, `[DEBUG]`, `[QUOTE]`), custom terminal output filter (`NoDebugTerminalFilter`) suppressing `DEBUG` lines from terminal stdout while allowing `INFO`/`SUCCESS`/`WARN`/`ERROR`/`QUOTE` logs, and isolated high-frequency telemetry logging to `wrapper_telemetry.log` capped at 500ms (2Hz). Implemented `QuoteEngine` service (`src/gui_v2/services/quote_engine.py`) handling event-driven personality quotes loaded from `src/.loading_quotes.json` ('boot', 'connect', 'profile_change'), low-frequency 7.5-minute idle rotation timers, and `logger.quote(...)` output. Integrated `TransitionOverlayWidget` (`src/gui_v2/widgets/transition_overlay_widget.py`) with `QuoteEngine` for loading quotes. Created `diagnostics/package_report.py` log export filter utility automatically stripping all `[QUOTE]` tagged lines from log files prior to packaging into `issue_report.zip`, and updated `generate_issue_report.bat` Step 7.
- **Hardware Chords DInput Mode & Auto Fallback Fix:** Disabled `HardwareChordEngine` processing in DInput mode (`src/hardware_chords.py`, `src/main.py`), preventing XInput hardware chord evaluation from zeroing out native DInput extra button inputs (`M1`, `M2`, `L4`, `R4`) and restoring dashboard pill illumination and remapped action execution. Added active runtime backend tracking via `status.json` (`src/main.py`), enabling `AdvancedView.check_backend_mode()` (`src/gui_v2/views/advanced_view.py`) and `get_extra_button_actions` (`src/button_matrix.py`) to detect when `mode = auto` falls back to DInput at runtime and hide the Hardware Chords group box widget accordingly. Updated dashboard extra button header to `"EXTRA BUTTONS"`.
