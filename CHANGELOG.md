...
- **HidHide Support Guide (Double Input Prevention)**: Added comprehensive documentation for setting up Nefarius HidHide using its official GUI (`HidHideClient.exe`). Users can whitelist `python.exe` / `run_wrapper.bat` and hide physical gamepads once at the OS level, eliminating double input in games while keeping UR-XD running safely without administrative driver overhead.
