...
- **Native GUI Calibration Wizard Overhaul:** Completely rewritten `NativeCalibrationWizardDialog` (`src/gui_v2/dialogs/calibration_wizard_dialog.py`). Features 15-second high-frequency XInput C-API auto-detection as Step 0 with countdown timer, constant polling, and an explicit **"Skip / DInput Mode"** button. Implemented thread-safe HID report signal delivery (`hid_report_received`), real-time raw HID byte/bitmask XOR delta detection (`curr ^ base`) for digital button inputs, mandatory button release waiting (`waiting_for_button_release`), 800ms-1.0s prompt reading cooldowns, analog noise filtering (`len(byte_history) > 3`), already-mapped bitmask protection, live analog stick min/max range and center tracking for stick axes (`lx`, `ly`, `rx`, `ry`, `lt`, `rt`), layout selection (`Xbox`, `PlayStation`, `Nintendo`), skip button options, and automatic profile generation in `profiles/{VID}_{PID}.json`.
