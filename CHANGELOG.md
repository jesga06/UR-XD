...
### 🎮 User-Facing Changes
- **HidHide Integration (Double Input Prevention)**: Automated interop with Nefarius HidHide to completely hide physical gamepads from Windows games while UR-XD is active. Includes automatic process executable whitelisting, dynamic device instance path cloaking (`normalize_device_path`), system environment diagnostic scanner (`diagnostics/07_hidhide_audit.py`), and crash-recovery safeguards (`recover_orphaned_cloaks`) that automatically restore devices on startup if an improper exit occurs.
