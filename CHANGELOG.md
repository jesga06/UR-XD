...
- **Dual-Save Target Buttons in Recorder Modal:** Rebuilt `KeyRecorderDialog` with **`Save Standard`** (purple) and **`Save Shift Map`** (green) action buttons, allowing instant targeted saving to either layer without reopening the modal.
- **Hardware Chords Engine Card Rows & Timing Modes:** Rebuilt Hardware Chords in `AdvancedView` to use dedicated card rows matching Screenshot 6 (`Chord:`, `Delayed:`, `Action:`, `Mode:` dropdown `auto`/`0ms`/`50ms`/`100ms`), added `? Hardware Chords Guide` popup, and updated `InputMapper` daemon to process delayed trigger chords and timing delays in real time.
- **Native Hover Tooltips Across Slider Controls:** Transformed individual setting `?` buttons in `TuningView` into clean native hover tooltips (`setToolTip`) attached directly to setting labels and sliders, removing visual clutter while providing instant setting explanations on mouse hover.
- **Legacy GUI Feature Audit:** Conducted a comprehensive audit of legacy GUI screenshots (v2.2.0, 7 screenshots) against current PySide6 implementation and published [gui_audit_legacy_vs_current.md](file:///C:/Users/jesse/.gemini/antigravity/brain/0a9b92a2-097f-4850-bdc6-eb26953b1289/gui_audit_legacy_vs_current.md) detailing spatial gamepad visualizer layout, dual-save recorder modal buttons, side-by-side 4-column remapping matrix, Hardware Chords card rows & mode dropdowns, and Shift Layer Settings placement.
...
- **Shift Layer Management Enhancements:** Fixed layer deletion in `RemappingView` to delete the currently selected layer by index (`del layers[idx]`), added a `✏️ Rename Layer` button with interactive `QInputDialog`, and added a dynamic warning banner for `HOME` button in `Hold` mode.
- **InputMapper Engine Remapping Restoration:** Updated `InputMapper.reload_config()` to load `config.data["mappings"]` and `config.data["block_xinput"]` into `layer_base` mappings and block sets, restoring active backend remapping execution.
- **Curve Dots & Circularity Hiding:** Restricted interactive control point dots strictly to `dotted custom` curve type, and hid the white circularity ring when circularity mode is `disabled`.
- **Shift Layer Recursion & Switching Fix:** Resolved a circular recursion bug in `RemappingView` where `load_config_values()` reset `combo_layers` index to `0`, ensuring seamless shift layer selection and layer creation.
- **Symmetrical Tuning Parameter Restoration & Live Curve Rendering:** Enforced 100% symmetrical configuration restoration in `TuningView` for Left/Right Sticks and Left/Right Triggers, wired all 10 trigger sliders to update response curve graphs live, enabled interactive control points for `dotted custom` curve preset, and bound custom math equation typing (`textChanged`) to repaint curve graphs instantly.
- **Global Theme Canvas Propagation & Connection Badge:** Dynamic theme changes now propagate color updates across all 8 canvas visualizer widgets (`JoystickVisualizerWidget`, `CurveGraphWidget`, `TriggerPullWidget`), and the connection badge dynamically reports controller connection status (`CONNECTED` vs `DISCONNECTED`).
- **Legacy Tuning & Response Curve Feature Parity:** Rebuilt `TuningView` matching Screenshots 2 & 3 with 6 dedicated sliders per stick/trigger (`Deadzone`, `Anti-Deadzone`, `Rest Deadzone`, `Warp Threshold / Power`, `Curve Factor`, `Sensitivity`), 7 selectable response curve types (`linear`, `exponential`, `aggressive`, `custom`, `dotted custom`, `sigmoid`, `bezier`), `Export Math` LaTeX exporters, `Reset` buttons, bright white circularity bounds rings on position radars, `TriggerPullWidget` vertical dual progress bars (Raw vs Tuned), and `Digital Trigger Mode` checkboxes.
