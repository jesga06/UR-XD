...
- **Debounced Theme Engine & Dropdown Sync Fix:** Decoupled disk I/O (`custom_theme.json` and `config.ini`) from live editing using a 300ms single-shot debouncing timer in `ThemeManager`, enabling 0ms latency and 60+ FPS UI updates during slider dragging and color tweaking. Added RAM caching for theme preset discovery (`get_available_themes`). Resolved Customization tab dropdown selection mismatches (`Active Theme`, `Button Color Source`, `Widget Background Source`, `Outline Color Source`) using native Qt signal blocking and `findData()` index matching. Fixed `widget_bg` and `outline` derived color calculations.
