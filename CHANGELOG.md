...
- **GUI Layer ID Matching Fix:** Updated `RemappingView._active_layer()` in `remapping_view.py` to match shift layers by explicit `userData` layer ID (`layer["id"]`) rather than list index, preventing dropdown reordering from mutating the wrong layer dictionary.
