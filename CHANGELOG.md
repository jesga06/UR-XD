...
- **Animated Transition Overlays:** Smooth color-interpolated overlay fades (250ms in, 900ms hold, 250ms out) with a rotating vector loading spinner and event-driven personality quotes on boot, device connect, and profile changes.
- **Dynamic Connection Dashboard:** The Dashboard dynamically switches between a USB HID device picker (waiting for connection) and live telemetry monitoring (once connected). The app automatically resolves profiles by checking XInput mode → existing local profiles → community database → native calibration wizard, in that order, without manual intervention.
- **Advanced & Utilities Views:** Fully implemented Hardware Chords Builder with input suppression, Macros Studio with interactive sequence recorder, 3 execution modes (`one_shot`, `toggle`, `hold`), live telemetry performance counters (polling rate, average processing latency, max latency delta), diagnostic suite launchers, and asynchronous Community Profiles Fetcher — all with full feature parity against the old interface.
- **Media Key Capture:** The keyboard remapping recorder now captures Windows hardware media keys (`media_volume_up`, `media_volume_down`, `media_volume_mute`, `media_play_pause`, `media_next`, `media_previous`, `media_stop`) directly.
...
- **Shift Layer Vibration Feedback (XInput Mode):** Tactile haptic feedback on shift layer transitions, with timeline-style vibration profiles (e.g. `RM[30% @ 0ms, dur=1500ms]`), automatic in-game rumble un-hijacking, and a waveform builder in the Advanced tab. *(Note: Seeking tester feedback across different XInput controller models.)*
- **System Tray Context Menu — Fully Implemented:** All tray icon actions are now fully operational: **Open Config**, **Pause Interception** (passes inputs through without remapping), **Reload Configuration**, **Show Console**, and **Exit**.
- **Tooltip Auto-Dismiss:** Hover tooltips and infobox text now automatically dismiss when the window loses focus, is minimized, or when any widget interaction occurs — preventing floating tooltip artifacts.
