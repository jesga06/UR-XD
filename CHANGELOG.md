...
## [2.3.0] - 2026-07-23 *(Unreleased Beta)*

### 🖥️ PySide6 GUI — Complete Interface Overhaul
The legacy CustomTkinter interface has been completely replaced with a modern, hardware-accelerated native **PySide6** application. The new GUI is substantially smoother, more responsive, and visually far more polished than the previous version.

- **Redesigned From Scratch:** Dashboard (with live device picker, animated connection overlay, and real-time telemetry), Remapping, Tuning, Advanced, Utilities, and Customization tabs — all rebuilt entirely in PySide6 with full feature parity against the legacy interface and significantly improved aesthetics.
- **Native Calibration Wizard:** The command-line calibration process is now integrated directly into the GUI as a polished multi-step dialog. Includes undo/skip navigation at every step, live interactive stick radar verification panels right after axis calibration, and automatic profile generation without leaving the application.
- **Selective Input Calibration:** A **"🎯 Selective Calibration"** button on the Dashboard header lets you recalibrate specific inputs (individual buttons, thumbsticks, triggers, D-Pad, or extra back paddles) on-the-fly without repeating full device setup. Supports entering new custom extra button names (e.g. `C`, `Z`, `P1`) directly in the dialog.
- **Interactive Response Curve Editors:** Drag-and-drop control-point curve editors (2–8 adjustable dots) per stick and trigger card, with live decimal slider readouts, an inline circularity mode selector, a "↺ Reset Defaults" button per card, a LaTeX formula copy button, and a Color Guide signal legend popup.
- **Advanced Dynamic Theme Engine:** Pick 4 base colors (Window Background, Accent #1 for hardware inputs, Accent #2 for virtual outputs, Text Color) — the engine automatically derives all UI surface tones (widgets, graphs, buttons, outlines) mathematically. Ships with 7 built-in presets (Default Neon Purple, Cyber Orange, Emerald Mint, Crimson Red, Ocean Blue, Midnight White, Solar Yellow), full user theme management (Save, Rename, Copy, Delete), JSON theme import/export, and a live interactive preview panel.
- **Sandbox Theme Editing Architecture & Overwrite Guards:** Introduced local staging tokens (`staging_changed`) for the Customization tab and live preview panel, completely isolating live color/slider edits from global app QSS styling for zero-lag 60+ FPS preview rendering. Editing any control automatically switches the Active Theme dropdown to `"Custom Theme*"`. Added `"⚡ Apply App-Wide"` and `"↩ Discard Edits"` toolbar buttons. Added collision detection (`theme_exists`) enforcing system preset overwrite protection and interactive confirmation prompts before overwriting existing custom user themes.
- **Hardware Chords DInput Mode Isolation:** Hardware Chords are now correctly disabled in DInput mode, restoring native DInput extra button inputs (M1, M2, L4, R4), dashboard button illumination, and remapped action execution.

### 🎮 User-Facing Changes
- **Single-Instance Enforcement & Process Lifecycle Fix:** Resolved socket binding issue on Windows where `SO_REUSEADDR` allowed duplicate processes to bind to port 48124 and spawn duplicate virtual XInput controllers. Single instance now uses `SO_EXCLUSIVEADDRUSE` on Windows, preventing multiple daemon instances. Fixed GUI window close (`X`) to cleanly exit the process and release sockets.
- **PySide6 HidHide Assistant (Double Input Prevention)**: Integrated interactive HidHide controls directly into the PySide6 Utilities view (`src/gui_v2/views/utilities_view.py`). Features automatic `sys.executable` whitelisting (`[⚡ Whitelist App]`), parent USB & HID PnP container ID blocklisting (`[🛡️ Enable Cloak]`), instant global toggle (`[🔓 Disable Cloak]`), and automated software PnP device restart (`[🔄 Software Re-Plug]`) alongside clear plain-language user guidance.
- **Multiple Shift Remapping Layers:** Create, name, and switch between multiple custom shift layers, each with its own mappings and XInput block settings. Activate layers via a single shift key or a 2-key chord combination (e.g. `LB + RB`) in hold or toggle mode.
- **2-Key Priority Shift Layer Resolution:** Chord-activated layers (e.g. `HOME + RB`) take deterministic precedence over single-key layers when both keys are held simultaneously. Single-key layers still resolve cleanly when only the primary key is held.
- **Dynamic Per-Layer XInput Suppression:** Standard blocks (`Blk`) apply on the base layer; shift blocks (`S.Blk`) apply exclusively on active shift layers, preventing unintended XInput block bleed-through between layers.
- **Shift Layer Vibration Feedback (XInput Mode):** Tactile haptic feedback on shift layer transitions, with timeline-style vibration profiles (e.g. `RM[30% @ 0ms, dur=1500ms]`), automatic in-game rumble un-hijacking, and a waveform builder in the Advanced tab.
- **System Tray Context Menu — Fully Implemented:** All tray icon actions are operational: **Open Config**, **Pause Interception**, **Reload Configuration**, **Show Console**, and **Exit**.

### 🗑️ Deprecations & Removals
- **CustomTkinter GUI Purge:** The entire legacy CustomTkinter interface (`src/gui.py`), all Tkinter modal dialogs (including the old circularity calibrator), and legacy visual themes (`src/themes/*.json`) have been completely removed and replaced by the PySide6 application.
- **Interactive Layout Builder Removed:** The Tkinter-based drag-and-drop Layout Builder (`technical-stuff/interactive_layout_builder.py`) has been removed alongside the CustomTkinter GUI migration.
- **Legacy Pystray Removed:** The legacy `pystray` system tray icon has been replaced by PySide6's native `QSystemTrayIcon`, eliminating duplicate tray icon entries.

### 📚 Documentation Overhaul
- **Modular Documentation Directory (`docs/`):** Restructured project documentation into a dedicated `docs/` directory with focused topic guides: Getting Started, GUI User Guide, Calibration, HID Maps, Macros Studio, Hardware Chords, Troubleshooting, Architecture & Pipeline, Developer Guide, and FAQ.
- **Streamlined `README.md`:** Transformed the main README into an approachable landing page with a 5-minute quick-start guide, navigation index, and concise feature overview.

### ⚙️ Under-the-Hood Changes
- **Decoupled 1000Hz Telemetry Worker:** A dedicated thread-safe `QThread`-based UDP telemetry worker completely decouples high-frequency backend packet ingestion from GUI frame rendering, keeping the interface responsive at any polling rate.
- **Shift Layer Configuration Schema & Migration:** Shift layers are now stored under a `shift_layers` array in profile JSON files, with automatic backward-compatibility migration for existing single-shift profiles.
- **Zero-Allocation GUI Rendering:** Stick radar and trigger bar canvas widgets pre-allocate all drawing primitives (`QPen`, `QBrush`, `QPolygonF`), eliminating dynamic object allocation churn during high-FPS rendering.
- **Live Macro & Hardware Chord Synchronization:** Macros reload definitions on save, preventing stale executions. Newly added or deleted hardware chords immediately populate in the Dashboard button matrix and Remapping grid without requiring a restart.
- **Macro Hold Mode Release:** Releasing a button mapped to a `hold`/`loop` macro now immediately interrupts execution and resets state, clearly distinguishing hold mode from toggle mode.
- **1000Hz Pipeline & High-Frequency Telemetry Upgrade**: Upgraded native XInput backend polling to 1000Hz (1ms interval) with off-thread background HID enumeration. Upgraded PySide6 GUI telemetry stream worker to 240Hz default (for modern high-refresh displays) with dynamic 1000Hz sampling during calibration sweeps, capturing 1000Hz controller inputs without degree skips or UI lag.
- **Root-Cause Investigation (RCI) Stability Fixes**: Resolved tray-restore UI freezing via absolute project-root `status.json` path resolution, eliminated periodic 2-second input stutter by isolating kernel `hid.enumerate` calls to a background thread, and fixed circularity sweep polar degree gaps by shifting sample collection to 1000Hz telemetry slots.
- **Standardized Severity Logging System:** Custom log levels (`SUCCESS`, `QUOTE`), bracketed severity tags across all modules (`[INFO]`, `[SUCCESS]`, `[WARN]`, `[ERROR]`, `[DEBUG]`), terminal debug-level suppression, telemetry log isolation to `wrapper_telemetry.log` capped at 2Hz, and automatic personality quote line stripping from exported `issue_report.zip` packages.
- **Device Picker Worker Import Fix:** Added missing `QThread` to `PySide6.QtCore` imports in `device_picker_widget.py`, resolving a `NameError` crash during HID background device enumeration (`DeviceEnumWorker`).
