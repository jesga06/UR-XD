...
- **Legacy Remapping Matrix 4-Card Layout Restoration:** Rebuilt `RemappingView` matching Screenshot 1 into 4 distinct group cards (Face Buttons, Shoulders & Sticks, D-Pad, System & Extras). Each row features sleek outlined `QLineEdit` fields for direct manual mapping entry, `[R]` record modal triggers, `Block` native input suppression checkboxes, outlined shift map `QLineEdit` fields, and `S. Blk` shift block checkboxes.
